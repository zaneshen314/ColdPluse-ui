...
const CharityEventList = ({events, onJoinEvent, joinedIds}) => {
        ...
        <Grid container spacing={3}>
            {events.map((event) => (
                <EventCard
                    ...
                    buttonProps={{
                        text: joinedIds.includes(event.id) ? JOINED : JOIN_EVENT,
                        onButtonClick: joinedIds.includes(event.id) ? undefined : () => onJoinEvent && onJoinEvent(event.id),
                    }}
                />
            ))}
        </Grid>
